...
def register():
    ...
    if form.validate_on_submit():
        hashed_pas = bcrypt.generate_password_hash(
            form.password.data).decode('utf-8')
        user = User(username=form.username.data,
                    email=form.email.data, password=hashed_pas)
        db.session.add(user)
        db.session.commit()
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        hashed_pas = bcrypt.generate_password_hash(
            form.password.data).decode('utf-8')
        user.passsword = hashed_pas    
        db.session.commit()
        flash('Your password has been update! You are now able to login.', 'success')
        return redirect(url_for('users.login'))
    return render_template('reset_token.html', title = 'Reset passsword', form = form)                   